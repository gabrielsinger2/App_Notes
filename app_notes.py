...
with gr.Blocks(title="Suivi des notes") as demo:
    ...
    with gr.Tab("Ajouter une note"):
        with gr.Row():
            student_input = gr.Textbox(label="Nom de l'élève", placeholder="Ex: Alice Dupont")
            assignment_input = gr.Textbox(label="Contrôle / Chapitre", placeholder="Ex: Contrôle chapitre 1")
        with gr.Row():
            grade_input = gr.Textbox(label="Note", placeholder="Ex: 15.5")
            date_input = gr.Textbox(label="Date (optionnel, format AAAA-MM-JJ)", placeholder="Ex: 2025-11-16")
